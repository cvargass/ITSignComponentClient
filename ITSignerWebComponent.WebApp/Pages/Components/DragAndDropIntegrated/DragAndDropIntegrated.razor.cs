...
using ITSignerWebComponent.Core.Enums;
...
namespace ITSignerWebComponent.SignApp.Pages.Components.DragAndDropIntegrated
{
    public partial class DragAndDropIntegrated
    {
        [Parameter]
        ...
        private void HandleCheckRubricChange(ChangeEventArgs e)
        {
            bool isChecked = (bool)e.Value!;

            if (isChecked)
            {
                IsFirmaRubricaChecked = true;

                if (IsFirmaGrafoChecked)
                {
                    this.TypeSigning = TypeIntegratedSign.Composed;
                    this.dragItemsList.ToArray()[0].Name = "F. Compuesta";
                } else
                {
                    this.TypeSigning = TypeIntegratedSign.RubricQR;
                    this.dragItemsList.ToArray()[0].Name = "Rúbrica + QR";
                }
            }
            else
            {
                IsFirmaRubricaChecked = false;

                if (IsFirmaGrafoChecked)
                {
                    this.TypeSigning = TypeIntegratedSign.Grafic;
                    this.dragItemsList.ToArray()[0].Name = "Grafo";
                }
                else
                {
                    this.TypeSigning = TypeIntegratedSign.Rubric;
                    this.dragItemsList.ToArray()[0].Name = "Rúbrica";
                }
            }
        ...
        private void HandleCheckGrafoChange(ChangeEventArgs e)
        {
            bool isChecked = (bool)e.Value!;

            if (isChecked)
            {
                IsFirmaGrafoChecked = true;

                if (IsFirmaRubricaChecked)
                {
                    this.TypeSigning = TypeIntegratedSign.Composed;
                    this.dragItemsList.ToArray()[0].Name = "F. Compuesta";
                }
            ...
            {
                BytesImageGrafic = null;
                IsFirmaGrafoChecked = false;
        ...
        {
            this.AlldragItemsEnableList.Add(new() { Id = "drag-item-rubric", Class = "drag-item drag-item-rubric", Name = "Rúbrica + QR", UrlIcon = "./images/signature.png" });
