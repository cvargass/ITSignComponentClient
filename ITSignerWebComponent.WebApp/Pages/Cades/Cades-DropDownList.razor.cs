    ...
    public partial class Cades_DropDownList
    {
        ...
        protected async override void OnAfterRender(bool firstRender)
        {
            ...
            if (firstRender)
            {
                try
                {
                    var response = await _apiStoreFilesService.GetCadesPendingFiles(IdUser, IdApp);
                    ...
                    if (!string.IsNullOrEmpty(pem))
                    {
                        try
                        {
                            ...
                            if (await isValidCertificateAuthority())
                            {
                                ...
                                {
                                    var response = await _apiStoreFilesService.GetCadesPendingFile(fileName);

                                    if (response.Item1)
                                    {
                                        CadesFileDto.File = Convert.FromBase64String(response.Item2);

                                        var cms = await _jsRuntime.InvokeAsync<string>("generateCMSToEmbed", CadesFileDto.File);

                                        var flag = await _apiStoreFilesService.PostCadesSignedFile(new PostFileSignedDto { PdfGuid = fileName, PdfSignedBase64 = cms });
